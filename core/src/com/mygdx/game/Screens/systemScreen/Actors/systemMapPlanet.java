...
public class systemMapPlanet extends Image {

    private systemScreen2 screen;

    public systemMapPlanet(systemScreen2 screen, float width, float height, float Xcoord, float Ycoord){
        super(screen.getThrustAt().findRegion("PLANETBUTTON"));
        this.screen = screen;
        setWidth(width);
        setHeight(height);
        setX(Xcoord);
