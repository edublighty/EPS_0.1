...
public class systemMapGalaxy extends Image {

    private systemScreen2 screen;

    public systemMapGalaxy(systemScreen2 screen, float width, float height, float Xcoord, float Ycoord) {
        super(screen.getThrustAt().findRegion("GALBUTTON"));
        this.screen = screen;
        setWidth(width);
        setHeight(height);
        setX(Xcoord);
